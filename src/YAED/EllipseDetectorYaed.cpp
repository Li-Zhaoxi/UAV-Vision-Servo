...
void CEllipseDetectorYaed::SetParameters(cv::Size	szPreProcessingGaussKernelSize,
...
float CEllipseDetectorYaed::GetMedianSlope(std::vector<cv::Point2f>& med, cv::Point2f& M, std::vector<float>& slopes)
{
	// med		: vector of points
	// M		: centroid of the points in med
	// slopes	: vector of the slopes

	unsigned iNofPoints = (int)med.size();
...
void CEllipseDetectorYaed::GetFastCenter(std::vector<cv::Point>& e1, std::vector<cv::Point>& e2, EllipseData& data)
{
	using namespace cv;
	using namespace std;
...
void CEllipseDetectorYaed::DetectEdges13(cv::Mat1b& DP, VVP& points_1, VVP& points_3)
{
	using namespace cv;
	using namespace std;
...
void CEllipseDetectorYaed::DetectEdges24(cv::Mat1b& DN, VVP& points_2, VVP& points_4 )
{
	using namespace cv;
	using namespace std;
...
void CEllipseDetectorYaed::FindEllipses(	cv::Point2f& center,
											VP& edge_i,
											VP& edge_j,
											VP& edge_k,
											EllipseData& data_ij,
...
void CEllipseDetectorYaed::YAED_Ellipse_2_CV(std::vector<struct Ellipse>& ellipses, std::vector<cv::RotatedRect>& cvellipses)
{
	const int elp_num = (int)ellipses.size();
	if (elp_num == 0)
	{
		cvellipses.clear();
		return;
		...
		cvellipses.resize(elp_num);

	struct Ellipse *_in_data = ellipses.data();
	cv::RotatedRect *_out_data = cvellipses.data();

	for (int i = 0; i < elp_num; i++)
	{
		_out_data[i].center.y = _in_data[i]._xc;
		_out_data[i].center.x = _in_data[i]._yc;
		_out_data[i].size.width = 2*_in_data[i]._a;
		_out_data[i].size.height = 2*_in_data[i]._b;
		double dx = std::cos(-_in_data[i]._rad), dy = std::sin(-_in_data[i]._rad);
		if (dy < 0) { dy = -dy, dx = -dx; }
		_out_data[i].angle = float(-atan2(dx, dy) / CV_PI * 180);
	}

}

void CEllipseDetectorYaed::drawTransEllipses(std::vector<cv::RotatedRect> &elp, cv::Mat &ImgG)
{
	cv::Mat ImgT;
	cv::cvtColor(ImgG, ImgT, CV_GRAY2BGR);

	for (int i = 0; i < elp.size(); i++)
	{
		cv::RotatedRect temp;
		temp.center.x = elp[i].center.y;
		temp.center.y = elp[i].center.x;
		temp.size.height = elp[i].size.width;
		temp.size.width = elp[i].size.height;
		temp.angle = -elp[i].angle;
		ellipse(ImgT, temp, cv::Scalar(0, 0, 255), 2);
	}
	cv::imshow("TransElps", ImgT);
	cv::waitKey(1);
}